...
class Functions
{
    ...
    {
        if ($id == "") {
            $getData = "Select * from $this->student_details";
        } else {
            $getData = "Select * from $this->student_details where id=:id";
        }
    ...
    function deleteData($id)
    {
        ...
        $deleteData = "delete from $this->student_details where id=:id";
    ...
    function checkEmail($email)
    {
        ...
        $this->createTable();
        $checkEmail = "select * from $this->student_details where email=:email";
    ...
    function checkEmailAndPassword($email, $password)
    {
        ...
        $checkEmail = "select id,name,email,password from $this->student_details where email=:email";
        $stmt = $this->conn->prepare($checkEmail);
        $stmt->bindParam(":email", $email);
            ...
            $createBlogTable = "
                CREATE TABLE IF NOT EXISTS `posts` (
                    ...
                    `updated_at` timestamp NOT NULL DEFAULT current_timestamp() ON UPDATE current_timestamp(),
                     FOREIGN KEY (`user_id`) REFERENCES $this->student_details (`id`) ON DELETE NO ACTION ON UPDATE NO ACTION
    ...
    function getBlogData($id, $str)
    {
        try {
            if ($id == "" and $str == "") {  //&& $num == 0) {
                ...
                $getBlogData = "Select s.name,b.id,b.user_id,b.title,b.image,b.body,b.published,b.created_at from $this->blog_posts b inner join $this->student_details s on b.user_id=s.id where b.id=:id";
            }
    ...
    function getCommentData($bid)
    {
        try {
            ...
            $getBlogData = "select sd.name,c.id,c.bid,c.uid,c.comment,c.created_at,c.updated_at from $this->comment c inner join $this->student_details sd on sd.id = c.uid where bid=:bid order by created_at ASC";
            $stmt = $this->conn->prepare($getBlogData);
            $stmt->bindParam(":bid", $bid);
            $stmt->execute();
            $stmt->setFetchMode(PDO::FETCH_ASSOC);
